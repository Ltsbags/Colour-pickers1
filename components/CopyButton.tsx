...
interface CopyButtonProps {
  ...
  label?: string;
  className?: string;
  variant?: 'outline' | 'ghost' | 'solid' | 'badge';
  size?: 'sm' | 'md' | 'lg';
}
...
export function CopyButton({
  ...
  return (
    <button
      type="button"
      onClick={handleCopy}
      id={`copy-btn-${textToCopy.replace(/[^a-zA-Z0-9]/g, '')}`}
      className={`inline-flex items-center justify-center font-medium rounded-lg transition-all active:scale-95 cursor-pointer select-none ${sizeClasses[size]} ${variantClasses[variant]} ${className}`}
